...
class simple_html_dom_node {
    ...
    function makeup()
    {
        ...
        foreach ($this->attr as $key=>$val)
        {
            ...
            $ret .= $this->_[HDOM_INFO_SPACE][$init][0];
            //no value attr: nowrap, checked selected...
            if ($val===true)
                $ret .= $key;
            else {
                switch ($this->_[HDOM_INFO_QUOTE][$init])
                {
                    case HDOM_QUOTE_DOUBLE: $quote = '"'; break;
                    case HDOM_QUOTE_SINGLE: $quote = '\''; break;
                    default: $quote = '';
                }
                $ret .= $key.$this->_[HDOM_INFO_SPACE][$init][1].'='.$this->_[HDOM_INFO_SPACE][$init][2].$quote.$val.$quote;
            }
        }
...
//        $pattern = "/([\w-:\*]*)(?:\#([\w-]+)|\.([\w-]+))?(?:\[@?(!?[\w-]+)(?:([!*^$]?=)[\"']?(.*?)[\"']?)?\])?([\/, ]+)/is";
    ...
    function getElementById($identity) {return $this->find("#$identity", 0);}
    function getElementsById($identity, $idx=null) {return $this->find("#$identity", $idx);}
...
class simple_html_dom {
    ...
    protected function parse() {
        if (($set = $this->copy_until_char('<'))==='')
            return $this->read_tag();

        // text
        $node = new simple_html_dom_node($this);
        ++$this->cursor;
        $node->_[HDOM_INFO_TEXT] = $set;
